...
The first candidates for feature engineering were the latitude and longitude columns. As mentioned previously, coordinate values are obviously not of much use in the modelling process as the numbers themselves. But they do contain information that could be extracted for a range of useful predictors. To try and determine what kind of useful features could be created, I generated a heat map of the listings.
...
Another important factor regarding location in a city, is not only the postcode but also the loaction relative to areas of interest.  Drawing upon my previous knowledge of the city, some basic logic, and the insights from the EDA, I picked out five locations: the CBD, the MCG/Melbourne Sports Precinct, Chapel Street, St Kilda, and Fitzroy. These five areas contains the city's most visited tourist attractions, beach areas, and nightlife districts. After deciding on the locations I needed a distance metric. The Euclidean distance, which gives the distance as the crow flies, may seem like the obvious choice. But in city such as Melbourne, which has a grid-like road system, it's probably not reflective of the true distance needed to travel there. However, the Manhattan distance is perfect for this. After measuring the distance from each location for each observation, I now had five more feature that are much more useful that mere coordinate values. However, the five features are (as expected) highly correlated, all with 𝜌>0.9. To work around this, principal components analysis (PCA) was used. As seen on the chart below, the first principal component captured more than 97.5% and this became the feature for the final dataset, replacing the distances from the five individual locations; the remaining principal components were not considered.

<img src="images/Distance_PCA_scree_plot.png?raw=true"/>
